...
"""
Copyright (C) 2012  kirill Kulakov & Jose Carlos Granja & Mijail Rondon
...
def getOut():
  print "Para ver comandos usa --help"
  sys.exit(2)
...
def main():
  ...
  if FILENAME == "" and not ERASE_MODE:
    print "Necesitas seleccionar un archivo HEX con la opcion -i"
    getOut()
  ...
  if arduino.read() == 'H': 
	...
	if not ERASE_MODE:
	  ...
	  for i in range(0x0F):
	    if hexFile.fuseChanged(i):
	      ...
	      time.sleep(SLEEP_TIME)
	  print "\tLogrado"
	  print "Programando memoria flash...",
	...
	print "No se pudo borrar el chip."
	sys.exit(1)
    else:
      print "MCU no reconocido. Verifica la lista de MCU compatibles y/o verifica las conexiones."
      sys.exit(1)
  else:
